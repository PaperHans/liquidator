...
import db from "./db";
import { searchUserQuery, insertUserQuery2 } from "../utils/psqlUtils";
...
// init
const { CHAINSTACK_WSS, TABLE_USER_BALANCES } = process.env;
...
const init = async () => {
  ...
  contract.events
    ...
    .on("data", async (event) => {
      ...
      if (event.event === "Repay" || event.event === "Withdraw") {
