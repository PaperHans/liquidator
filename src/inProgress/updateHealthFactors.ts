...
const getHealthFactorForAccounts = async (_batchOfAccounts) => {
  for (let i = 0; i < 2; i += 1) {
    try {
      const healthFactorArr = await healthFactorContract.methods
        .healthFactors(_batchOfAccounts, token)
        .call();
      // map health factors to accounts
      const mappedHealthFactorArr = healthFactorArr.map((hf, idx) => ({
        accountAddress: _batchOfAccounts[idx],
        healthFactor: toNumber(hf),
      }));
...
export const removeAccounts = async (_accountsToRemove) => {
...
export const batchUpdateHealthFactor = async (_acctHealthFactorArr) => {
  const idArr = [];
  const queryValues = [];
  const toRemoveIdArr = _acctHealthFactorArr
    .filter(({ healthFactor }) => {
      return !(
        healthFactor < 3000000000000000000 && healthFactor > 100000000000000000
      );
    })
    .map(({ accountAddress }) => ({ accountAddress }));
  _acctHealthFactorArr.forEach(({ accountAddress, healthFactor }) => {
    if (
      healthFactor < 3000000000000000000 &&
      healthFactor > 100000000000000000
    ) {
  ...
  if (queryValues.length > 0) {
    ...
    const querySelect = `(SELECT UNNEST(array[${idArr.join(
      ", "
    )}]) AS ${keyName}, UNNEST(array[${queryValues.join(
      ", "
    )}]) AS ${valName}) AS data_table`;
...
const getHealthFactorForAccountsAscending = async (_batchOfAccounts) => {
  for (let i = 0; i < 2; i += 1) {
    try {
      const healthFactorArr = await healthFactorContract.methods
        .healthFactors(_batchOfAccounts, token)
...
// main loop function
export const loopAndUpdateAccounts = async (
  _accountsArr,
  ...
  for (let batchIdx = 0; batchIdx < batchCt; batchIdx += 1) {
    ...
    if (isDescending) {
      const acctHealthFactorArr = await getHealthFactorForAccountsDescending(
        batchOfAccounts
      );
    } else {
      const acctHealthFactorArr = await getHealthFactorForAccountsAscending(
        batchOfAccounts
      );
    }
